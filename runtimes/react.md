...
1.  **安装依赖**
    ...
    -   `@rive-app/react-canvas-lite` - 类似于 `@rive-app/react-canvas`，但 [更小](/runtimes/web/canvas-vs-webgl)。如果 Rive 图形不使用 [Rive 文本](/editor/text/overview.md)，建议使用此选项。
...
2.  **渲染 Rive 组件**
    ...
    有关 `<Rive />` 组件参数和返回值的更多信息，请参阅 [参数和返回值](/runtimes/react/parameters-and-return-values)。
...
3.  **使用 useRive Hook**
    ...
    **注意：** Rive 只有在 `<RiveComponent />` 被渲染后才会实例化，因为底层的 `<canvas>` 元素需要存在于 DOM 中。
    ...
    此外，请记住，canvas 的大小取决于它所在的容器。最初，它是 0x0。请向 `RiveComponent` 传递 `className` 或用适当大小的容器包裹 `RiveComponent`。有关 `useRive` 的参数和返回值的更多信息，请参阅 [此处](/runtimes/react/parameters-and-return-values)。此外，浏览后续运行时页面以了解如何控制动画播放、状态机等。
...
[从 v1 迁移到 v2](/runtimes/web/migrating-from-v1-to-v2)[参数和返回值](/runtimes/react/parameters-and-return-values)