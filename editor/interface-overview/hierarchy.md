...
父子关系是 Rive 的核心概念，它允许你以极小的代价创建复杂的层级式动画。[分组 (Groups)](/docs/editor/fundamentals/groups) 和 [骨骼 (Bones)](/docs/editor/manipulating-shapes/bones) 在 Rive 中都可以拥有子级。
...
要更改舞台上对象的绘制顺序，请在列表中将形状或分组向上或向下拖到另一个之上或之下。请注意，绘制顺序也会影响对象在布局 (Layout) 中的放置和处理方式。在此处了解更多信息：[此处](https://rive.app/docs/editor/layouts/layouts-overview)。
绘制顺序可以制作动画，但过程稍微深入一点。请在 [绘制顺序动画](https://rive.app/docs/editor/animate-mode/animating-draw-order) 页面阅读相关内容。

## [​](#right-click-menu) **右键菜单 (Right Click Menu)**

在层级面板中右键点击任何对象都会弹出一个针对不同对象的选项菜单。在菜单中，你可以找到复制/粘贴对象和样式、删除对象、将对象包裹在 [布局 (Layouts)](https://rive.app/docs/editor/layouts/layouts-overview) 和 [Solo (Solos)](https://rive.app/docs/editor/manipulating-shapes/solos) 中、显示依赖图、添加 [标签 (Tags)](https://rive.app/docs/editor/tagging)、翻转绘制顺序以及 [导出名称](https://rive.app/docs/editor/exporting/exporting-for-runtime) 的功能。
...
[## 导入资产 (Importing Assets)](https://rive.app/docs/editor/fundamentals/importing-assets)[## 音频事件 (Audio Events)](https://rive.app/docs/editor/events/audio-events)

# [​](#data-panel) 数据面板 (Data Panel)

数据面板是创建、组织和查看文件所有视图模型 (View Models)、枚举 (Enums) 和转换器 (Converters) 的地方。该面板分为三个独立空间：视图模型、枚举和转换器。在此处了解更多关于数据绑定 (Data Binding) 的信息：[此处](https://rive.app/docs/editor/data-binding/overview)。
