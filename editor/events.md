...
> *[原文档演示图由于官方服务器迁移已失效]* (描述: 添加新属性)
你需要为属性重命名，并选择其跟踪的值类型（数字、布尔值或字符串）。

### [​](#url) URL

当选择 `Open URL` 事件时，会出现额外的配置：
> *[原文档演示图由于官方服务器迁移已失效]* (描述: URL 配置示例)
...
> *[原文档演示图由于官方服务器迁移已失效]* (描述: 在时间轴关键帧上报告事件)
...
> *[原文档演示图由于官方服务器迁移已失效]* (描述: 在状态或过渡中报告事件)
